  ...
  useEffect(() => {
    const fetchRecipeForEdit = async () => {
      try {
        ...
        if (!response.ok) {
          throw new Error("Recipe not found or you are not authorized to edit it.");
        }
      ...
      <div className="max-w-3xl mx-auto p-8 space-y-6 bg-white rounded-xl shadow-lg">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div>
            ...
            <input type="file" id="recipeImage" onChange={(e) => setRecipeImage(e.target.files[0])} className="w-full mt-1 text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-emerald-50 file:text-emerald-700 hover:file:bg-emerald-100" />
          </div>
